...
locally at install time, no external QR service involved. The Wi-Fi QR
is a standard `WIFI:` join-only payload (no nonstandard combined
Wi-Fi+URL trick); the Help page also prints a plain-text `OPEN >
piratebox/` fallback right under it, so scanning that one code and
reading the printed hostname is sufficient by itself even without the
second QR. See [docs/OPERATIONAL-DECISIONS.md](docs/OPERATIONAL-DECISIONS.md)
(Phase 5; briefly simplified to a single code, then restored to two -
see "Two-QR PirateBox Onboarding Restored").
...
- **Offline Utility Library** (`/utility/`): a growing offline reference/utility section - radio reference, emergency/outage reference, first aid, maps &amp; local info, a document library, and cross-section search - all working without any Internet access, same as the rest of the site. Linked from the main navigation. All planned sections are built out (Stages 2-8). See docs/OPERATIONAL-DECISIONS.md for the staged plan and progress.
  - **Search** (`/utility/search/`): one search box across every section above (92 indexed items as of this writing - the live page always shows the real current count, computed, never hardcoded) - results link directly to the answer (e.g. `/utility/radio/#noaa-weather-radio` auto-opens and scrolls to that entry). Entirely client-side against a pre-built static index (`tools/build_search_index.py` rebuilds it) - no database, no server-side search process.
  ...
  - **Maps &amp; Location Reference** (`/utility/maps/`): a bundled World Reference Map (static offline SVG, country borders, built from Natural Earth's public-domain 1:110m data - always available, regardless of Travel Mode or whether Local Information is configured), coordinate/GPS/navigation basics (USGS/NOAA-sourced), plus an empty, ready-to-use *local/regional* map catalog framework - add a real map for this box's own area by dropping a file in `public/utility/maps/files/` and one entry in `data/utility/maps/catalog.json`, no PHP editing needed. No *local/regional* map data ships by default; the World Reference Map does.
  ...
  - **Document Library** (`/utility/library/`): a data-driven catalog framework for manuals/reference documents (radio, Pi/Linux/networking, electronics, vehicle/generator, equipment you own, etc.) - add a document by dropping the file in `public/utility/library/files/` and one entry in `data/utility/library/catalog.json`; `tools/check_library_catalog.py` verifies they agree. Ships with no documents - nothing bundled or downloaded.
  - **Field Tools** (`/utility/fieldtools/`): time/date tools (current local &amp; UTC time, ISO-8601, Unix timestamp, elapsed-time calculator, this device's time-source status), unit conversion (temperature, distance, mass, volume, speed, pressure, storage, percentage, electrical/battery runtime), and a coordinate (decimal degrees &harr; DMS) converter - a field-instrument role for this device, not just file-sharing. Interactive tools need JavaScript; the current-time reading and reference tables work with it off. See docs/FIELD-TOOLS-DESIGN.md.
