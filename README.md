...
- **Offline Utility Library** (`/utility/`, in progress): a growing offline reference/utility section - radio reference, emergency/outage reference, first aid, maps &amp; local info, a document library, and cross-section search - all working without any Internet access, same as the rest of the site. Linked from the main navigation. Radio Reference is built out (Stage 2); Emergency/First Aid/Maps/Library/Search remain placeholders. See docs/OPERATIONAL-DECISIONS.md for the staged plan and progress.
