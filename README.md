...
- **Offline Utility Library** (`/utility/`, in progress): a growing offline reference/utility section - radio reference, emergency/outage reference, first aid, maps &amp; local info, a document library, and cross-section search - all working without any Internet access, same as the rest of the site. Linked from the main navigation. Radio Reference and Emergency Reference are built out (Stages 2-3); First Aid/Maps/Library/Search remain placeholders. See docs/OPERATIONAL-DECISIONS.md for the staged plan and progress.
