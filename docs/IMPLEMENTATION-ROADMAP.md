...
| ESP32-S3 hardware/sensor supervisor | **PARTIALLY IMPLEMENTED (2026-09-07)** - identity commissioned, full firmware+daemon stack built and live-validated; no sensors migrated/added yet beyond the on-die temp sensor | PlatformIO/Arduino firmware (NDJSON protocol, task watchdog, `temp_internal` capability), Pi daemon (`piratebox_esp32_supervisor.py`, stable `/dev/serial/by-id/` discovery), `HARDWARE_SIGNALS` registration, Environment UI section, 29+28 passing unit tests, factory firmware backed up before first custom flash - all built and validated on the real commissioned board same day. Requires an externally-powered USB hub in the current prototype topology (unrelated to, but correlated with, the Pi's own still-unresolved `0x50005` undervoltage condition) | BH1750 migration (Pi->ESP32) is the recommended next physical step - not performed, requires operator rewiring (a genuine gate, see `docs/ESP32-SUPERVISOR-DESIGN.md` §16). DS18B20/BME280/INA226/power-supervision remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md` |
