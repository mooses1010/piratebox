...
| ALFA AWUS036ACM (MT7612U) upgrade | HARDWARE VALIDATION IN PROGRESS - AP capability empirically verified, operator RF gate pending | `lsusb` confirms `0e8d:7612`/`mt76x2u` bound; isolated test AP (`PirateBox-ALFA-Test`) reached `AP-ENABLED` and beaconed stably, no new power/USB errors under load; `wlan0` confirmed untouched throughout | operator: associate a phone/laptop to the isolated test SSID and report back (see gate below); do not migrate production until that passes | OPERATIONAL-DECISIONS.md "AWUS036ACM Hardware Validation Round" |
