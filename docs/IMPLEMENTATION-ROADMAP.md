...
| ALFA AWUS036ACM (MT7612U) upgrade | HARDWARE/DRIVER/AP-ASSOCIATION VALIDATED - not yet production AP | `lsusb` confirms `0e8d:7612`/`mt76x2u` bound; isolated test AP (`PirateBox-ALFA-Test`) reached `AP-ENABLED`; operator's phone/laptop completed full WPA2 4-way handshake 3x (`hostapd` log + live `iw station dump`), no new power/USB errors across the whole test window; `wlan0` confirmed untouched throughout; test AP cleanly torn down after | a deliberate, separate migration decision (not automatic) - would need real DHCP service on the serving interface, persistent interface naming, and the carried-forward power caveat budgeted in | OPERATIONAL-DECISIONS.md "AWUS036ACM Hardware Validation Round" |
