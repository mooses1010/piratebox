...
| hostapd auto-recovery: restart trigger unreliable | **IMPLEMENTED + DEPLOYED + LIVE-VERIFIED (2026-09-15)** - first two mechanisms both root-caused as genuine no-ops, not merely "unreliable"; third confirmed working on a real disconnect/reconnect | Two restart-trigger mechanisms tried and disproven with direct live evidence, in order: (1) `ENV{SYSTEMD_WANTS}` on the udev rename rule - the kernel's `move` uevent (which finalizes the renamed device) never re-matches the rule's `ACTION=="add"` condition, confirmed via `udevadm test --action=add` vs. `--action=move` plus live `Wants=`/property checks showing nothing ever reached the device unit. (2) A `PathExists=/sys/class/net/pb-ap` `.path` unit (this row's own prior "most likely replacement" suggestion) - fired once on its own startup existence check, then failed to fire on a real, operator-performed physical unplug/replug test (hostapd stayed dead 45+ seconds after `pb-ap` was confirmed back via the kernel log), likely a kernfs/sysfs inotify limitation, not proven at the kernel-internals level. (3) **Confirmed working, twice over**: `RUN+="/usr/bin/systemctl --no-block start hostapd.service"` added directly to the same udev `add`-event rule line that has reliably renamed the interface across every blip observed - runs inside udev's own already-proven event processing, with no dependency on a later event or a virtual filesystem's inotify behavior. A PC lockup interrupted the session between merging this fix and redeploying it, during which the still-live (disproven) `.path` unit caused a real, unattended ~33-minute outage on the next ALFA blip - direct additional confirmation of failure mode #2, found on session resume. Redeployed (`diff`-confirmed byte-identical to repo), then a deliberate real unplug/replug confirmed `RUN+=` firing hostapd with zero manual intervention: disconnect 12:42:12.311 -> hostapd stopped 12:42:12.585 -> device back 12:42:31.711 -> renamed 12:42:33.251 -> udev `RUN+=` starts hostapd 12:42:33.977 -> `AP-ENABLED` 12:42:35.255, ~23s total, no `sudo`/`systemctl start` anywhere in the journal for that window. `override.conf`'s stop-half (`BindsTo=`/`ConditionPathExists=`) unchanged, reconfirmed working a fourth time. 16/16 regression tests pass. | none | `docs/OPERATIONAL-DECISIONS.md` 2026-09-15 dated entry, its same-day "`.path` unit also failed" update, and its closeout entry; 2026-09-05 incident entry for prior history |
| AP health monitoring: process-liveness blind spot | **DOCUMENTED ONLY, not yet fixed** - real gap found and confirmed 2026-09-05 | Incident: the ALFA disconnected/re-enumerated at 12:15 PDT; hostapd's process survived (bound to the now-gone wiphy) and stayed `active`/`ActiveState=active` for hours with zero beaconing - `/run/piratebox/status.json`'s `services.hostapd` field (and any other check that only asks systemd for a service's `ActiveState`) reported healthy the entire time, because it only checks that the *process* is running, never whether the AP is actually up/beaconing/reachable. The runtime-reattachment fix (see `docs/OPERATIONAL-DECISIONS.md`'s 2026-09-05 incident entry) fixes the underlying hostapd behavior; it does **not** fix this monitoring blind spot - a future recurrence of a *different* AP-health fault that still leaves the process "active" would be invisible to `status.json` again | Needs `status.json` (or a new companion check) to actively probe real AP health - e.g. `pb-ap` operationally UP with carrier, or a real client actually associated - not just `systemctl is-active hostapd`. Deliberately not implemented as part of the 2026-09-05 incident response (a larger, separate change than that incident's fix); scope it as its own small round when picked up | `docs/OPERATIONAL-DECISIONS.md` 2026-09-05 incident entry |
...
| GPIO25 shutdown button | IMPLEMENTED+DEPLOYED+LIVE-VERIFIED | Live process `piratebox_button_daemon.py` running; real hardware power-cycle test passed (Stage 29 real-hardware confirmation). Now a 3-gesture control: ~4s hold (shutdown, unchanged), single tap (temporary OLED status-check override), and double tap (toggles Silly Mode) - see the OLED row above and `docs/OPERATIONAL-DECISIONS.md` "OLED cadence rebalance + short-press status-check override" / "Double-tap Silly Mode toggle (physical button)" for the full gesture-discrimination design and long-hold safety guarantees | none | Stage 29 |
| ESP32-S3 hardware/sensor supervisor | **IMPLEMENTED + PIRATEBOX-WIDE INTEGRATED + HISTORICAL GRAPHING (2026-09-08)** - firmware `0.2.0`; five DS18B20 probes wired, discovered, physically commissioned (durable ROM<->physical_index 1-5 identity), genuinely woven into the rest of the product (admin capability table, OLED fault tier, Progression), and now has bounded historical graphs on the Environment page | Everything from prior checkpoints remains live. 2026-09-08 hardware-awareness round: a shared sensor-health classifier (`piratebox_hardware_health.py`, mirrored in PHP) reuses `capability_state.php`'s existing `NOT_INSTALLED`/`AVAILABLE`/`DEGRADED`/`UNAVAILABLE`/`UNKNOWN` vocabulary instead of the ~5 independent ad hoc reductions the audit found; three new admin rows (`esp32_supervisor`/`ambient_light`/`ds18b20_probes`, all Optional-layer, never able to make Core look dead); `piratebox_ds18b20_roles.py` gained a `commissioned`/`physical_index` identity pair (independent of naming) plus a reserved `role` field for a genuinely future functional classification; Environment page shows every commissioned probe (named or generic "Probe N"), fixed a real 10x bug in its "time ago" JS; OLED's fault/warning tier gained a hardware-problem signal at the exact same priority the chronic Pi undervoltage warning already uses; two new `HARDWARE_SIGNALS` feed one new spoiler-safe achievement (content not detailed here). A pre-existing, unrelated deployment gap was found and fixed live: the installed systemd unit was missing `StateDirectory=piratebox-esp32` (added in the prior phase's source but never actually deployed), so the DS18B20 roles store had never actually existed on this Pi - fixed, verified. All five probes' permanent identities recorded durably (Probe 1 `28fd856b0000003b` .. Probe 5 `28a50d01000000ca`) - no names/roles assigned, left to the operator. **Same-day follow-up: lightweight sensor history/historical graphing** (`docs/ESP32-SUPERVISOR-DESIGN.md` §22) - a new oneshot sampler (`piratebox_history_sampler.py`, triggered every ~5 min by a new `piratebox-history-sample.timer`, no new sensor polling) records only currently-valid readings into a bounded, flat-JSON, no-database store (`/var/lib/piratebox-history/`, one file per PERMANENT signal id, raw/hourly/daily tiers, 7d/90d/1yr retention); the Environment page gained a "History" section (Ambient Light, Temperature Probes, ESP32 Chip Temperature - 6h/24h/7d/30d ranges) rendered by a hand-rolled ~150-line canvas chart (no vendored library, no CDN); DS18B20 history is ROM-keyed (permanent) while the public API/UI only ever show "Probe 1".."Probe 5" via a dedicated slug-map bridge. Deployed via a new one-time installer (`setup_piratebox_history.sh`) and live-verified: a real first sample recorded all 7 signals, the history endpoint returns real data with zero ROM leakage, 32 KB total storage footprint, zero regressions anywhere. 588 Python tests (50 new), 493 PHP assertions (41 new), all passing | Naming/role assignment for the five probes - intentionally left to the operator. BME280/INA226/power-supervision/enclosure remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md` §21-22, `docs/OPERATIONAL-DECISIONS.md`, `docs/CHECKPOINTS.md` |
...
| OLED (SSD1306, I2C) | IMPLEMENTED+DEPLOYED+LIVE-VERIFIED (2026-09-03); Silly Mode + Progression added 2026-09-04; Captain's Log web profile added 2026-09-04; cadence rebalance + short-press status-check added 2026-09-04; double-tap Silly toggle added 2026-09-04; Expression Engine v2 added 2026-09-04; Distance/Glance Display added 2026-09-04 | I2C1 enabled (`dtparam=i2c_arm=on` + `i2c-dev` module, both persistent); `i2cdetect -y 1` confirms the display at 0x3C; a real frame written and visually confirmed by the operator; `piratebox-oled.service` running, four-page rotation (Status/Time/Network/Health) confirmed live on the physical display. Silly Mode: optional, user-toggled (`piratebox-silly {on,off,status}` **or now a double tap on the physical shutdown button - both control the exact same state**), default off, expressive face/personality display state subordinate to Emergency Mode/faults, now alternating with a genuine ~15s status interlude and an ~8s large-format glance page every ~53s (not a one-tick peek). Progression: persistent XP/level/title/achievement/lifetime-history layer underneath Silly Mode (`piratebox_progression.py`, durable at `/var/lib/piratebox-oled/`), cosmetic-only rewards, anti-farming and privacy by construction, with a currently-empty `HARDWARE_SIGNALS` extension registry for the DS3231/INA226/BME280/DS18B20/BH1750/RGB/GPS row below. Captain's Log: read-only web page (`/utility/captains-log/`) presenting Progression to visitors, via a new root-published public export (no filesystem permission loosened; one narrow named-file `open_basedir` addition instead). Short-press status-check: the existing GPIO25 shutdown button's short press (below its 4.0s hold threshold) now temporarily overrides Silly Mode with real status pages for ~15s, via a plain timestamp signal file - the long-hold shutdown path itself is unchanged and the two are mutually exclusive by construction (tested). Double-tap Silly toggle: a second short tap arriving within a 400ms window of the first toggles Silly Mode directly (button daemon writes the same file the CLI does) and shows a one-shot "SILLY MODE ON/OFF" confirmation; a lone single tap still produces exactly one (slightly delayed) status-check request, a long hold is unaffected, and all three gestures are mutually exclusive by construction (tested). Expression Engine v2: face/animation drawing moved to its own module (`piratebox_expressions.py`); a richer eye/decoration vocabulary and asymmetric-eye support; a small, bounded multi-frame animation system played as a brief synchronous burst (display cadence itself unchanged); Progression's own personality weights now actually bias which ambient variant shows; a new operator-only `piratebox-silly preview` command demos a fixed, non-secret sampler on the real display. Distance/Glance Display: a new `piratebox_glance.py` module renders a handful of large-format, one-question-at-a-time pages (CPU, RAM, disk, client count, uptime, time, chronic power warning) via a small deterministic, context-aware eligibility/weighting scheduler, interleaved into BOTH the Silly-on cadence (evolved from 2 to 3 phases) and a new, separate Silly-off cadence - never a new operational mode, never required to see it; `piratebox-silly glance-preview` is the equivalent operator-only preview command, independent of Silly Mode's own on/off state - see `docs/OPERATIONAL-DECISIONS.md` → "OLED Silly Mode" / "PirateBox Progression" / "Captain's Log web profile" / "OLED cadence rebalance + short-press status-check override" / "Double-tap Silly Mode toggle (physical button)" / "Expression Engine v2" / "Distance / Glance Display" | none for the OLED itself - GPIO22/23 (cycle/wake buttons) remain BLOCKED BY HARDWARE, see the row below | HARDWARE-INTEGRATION-DESIGN.md §12, PHYSICAL-CONTROL-UX-DESIGN.md §1/§4.1, OPERATIONAL-DECISIONS.md "OLED Silly Mode" / "PirateBox Progression" / "Captain's Log web profile" / "OLED cadence rebalance + short-press status-check override" / "Double-tap Silly Mode toggle (physical button)" / "Expression Engine v2" / "Distance / Glance Display" |
