...
| ESP32-S3 hardware/sensor supervisor | **IMPLEMENTED + PIRATEBOX-WIDE INTEGRATED (2026-09-08)** - firmware `0.2.0`; five DS18B20 probes wired, discovered, physically commissioned (durable ROM<->physical_index 1-5 identity), and as of this round genuinely woven into the rest of the product (admin capability table, OLED fault tier, Progression) rather than an isolated subsystem | Everything from prior checkpoints remains live. 2026-09-08 hardware-awareness round: a shared sensor-health classifier (`piratebox_hardware_health.py`, mirrored in PHP) reuses `capability_state.php`'s existing `NOT_INSTALLED`/`AVAILABLE`/`DEGRADED`/`UNAVAILABLE`/`UNKNOWN` vocabulary instead of the ~5 independent ad hoc reductions the audit found; three new admin rows (`esp32_supervisor`/`ambient_light`/`ds18b20_probes`, all Optional-layer, never able to make Core look dead); `piratebox_ds18b20_roles.py` gained a `commissioned`/`physical_index` identity pair (independent of naming) plus a reserved `role` field for a genuinely future functional classification; Environment page shows every commissioned probe (named or generic "Probe N"), fixed a real 10x bug in its "time ago" JS; OLED's fault/warning tier gained a hardware-problem signal at the exact same priority the chronic Pi undervoltage warning already uses; two new `HARDWARE_SIGNALS` feed one new spoiler-safe achievement (content not detailed here). A pre-existing, unrelated deployment gap was found and fixed live: the installed systemd unit was missing `StateDirectory=piratebox-esp32` (added in the prior phase's source but never actually deployed), so the DS18B20 roles store had never actually existed on this Pi - fixed, verified. All five probes' permanent identities recorded durably (Probe 1 `28fd856b0000003b` .. Probe 5 `28a50d01000000ca`, from the prior phase's live warming-test correspondence) - no names/roles assigned, left to the operator. 538 Python tests (73 new this round), 452 PHP assertions (21 new this round), all passing; deployed and live-verified end-to-end (admin classification, Environment page rendering with zero ROM leakage, the new achievement genuinely unlocking, zero regressions to I2C/Core services/the known chronic undervoltage condition) | Naming/role assignment for the five probes - intentionally left to the operator. BME280/INA226/power-supervision/enclosure remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md` §21, `docs/OPERATIONAL-DECISIONS.md`, `docs/CHECKPOINTS.md` |
