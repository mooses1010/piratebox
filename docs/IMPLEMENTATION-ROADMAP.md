...
| hostapd auto-recovery: udev `SYSTEMD_WANTS` restart trigger unreliable | **ROOT-CAUSED + FIXED IN REPO (2026-09-15), NOT YET LIVE-INSTALLED** | Root cause established directly (not guessed): `NAME=`-driven interface renames fire a kernel `move` uevent separate from the original `add` event our rule matched, and `ENV{SYSTEMD_WANTS}` set only during `add`-event rule processing never survives into the `move` event's udev database entry that systemd's device-unit management actually reads - confirmed via `udevadm test --action=add` (property computed) vs. `--action=move` (rule doesn't match at all) plus live `Wants=`/property checks on a real, already-renamed `pb-ap` showing nothing. Fixed by removing the dead `ENV{SYSTEMD_WANTS}` clause and adding `etc/systemd/system/piratebox-hostapd-recovery.path` (`PathExists=/sys/class/net/pb-ap` -> `Unit=hostapd.service`) - exactly this row's own prior "most likely replacement" suggestion. `override.conf`'s stop-half (`BindsTo=`/`ConditionPathExists=`) is unchanged and reconfirmed working on two more real ALFA blips today. 18/18 regression tests pass (`tools/test_hostapd_recovery_config.py`, extended). | Staged and committed to `main`; installing the new systemd unit + reloading udev rules is a live system-config change outside this project's narrow sudo grants, so it stops for the operator (exact commands in `docs/OPERATIONAL-DECISIONS.md`'s dated entry). Once installed, still needs a real or supervised ALFA replug to confirm unattended recovery end-to-end - not yet observed with this specific mechanism | `docs/OPERATIONAL-DECISIONS.md` 2026-09-15 dated entry ("hostapd pb-ap auto-recovery: udev SYSTEMD_WANTS root-caused and replaced with a .path unit"), 2026-09-05 incident entry for prior history |
