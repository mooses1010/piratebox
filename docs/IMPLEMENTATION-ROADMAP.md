...
| ESP32-S3 hardware/sensor supervisor | **PARTIALLY IMPLEMENTED (2026-09-07, DS18B20 phase)** - firmware `0.2.0` flashed and deployed; the actual hardware inventory turned out to be FIVE DS18B20 probes (corrected from an initial four), all five wired, discovered, validated live, and now physically commissioned (each ROM matched to its physical probe) | Everything from the prior checkpoint remains live (`temp_internal` + `bh1750` capabilities, Environment UI, `HARDWARE_SIGNALS`). DS18B20: firmware/daemon/commissioning-CLI/Environment-UI/OLED-glance-page all built, deployed, and validated against real hardware - all five ROMs discovered (family `0x28` each), independently changing live readings confirmed over time, zero malformed lines, `bus_ok: true`, zero regressions in BH1750/`temp_internal`/I2C/core services. Ambient-light-driven OLED auto-brightness deployed and confirmed computing correct real values from live sensor data (though the SSD1306's own applied-contrast register can't be read back in software - full physical confirmation needs a visual check). A new `identify` mode (baseline + live delta comparison) was added to `tools/ds18b20_commission.py` so the operator didn't have to manually compare five similar temperature streams by eye; using it, all five ROMs were matched to their physical probes by observed warming, one at a time, each confirmed by its own unambiguous sustained rise. None of the five probes are named/assigned a role yet - that decision is the operator's alone. 25+6+6+19+8 new Python tests, 13 new PHP assertions, all passing; full project-wide regression clean | Naming/role assignment for the five now-identified probes - not yet done, and intentionally left to the operator. BME280/INA226/power-supervision remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md`, `docs/CHECKPOINTS.md` |
