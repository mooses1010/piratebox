...
| hostapd auto-recovery: restart trigger unreliable | **THIRD MECHANISM FIXED IN REPO (2026-09-15), NOT YET LIVE-VERIFIED** - first two both root-caused as genuine no-ops, not merely "unreliable" | Two restart-trigger mechanisms tried and disproven with direct live evidence, in order: (1) `ENV{SYSTEMD_WANTS}` on the udev rename rule - the kernel's `move` uevent (which finalizes the renamed device) never re-matches the rule's `ACTION=="add"` condition, confirmed via `udevadm test --action=add` vs. `--action=move` plus live `Wants=`/property checks showing nothing ever reached the device unit. (2) A `PathExists=/sys/class/net/pb-ap` `.path` unit (this row's own prior "most likely replacement" suggestion) - fired once on its own startup existence check, then failed to fire on a real, operator-performed physical unplug/replug test (hostapd stayed dead 45+ seconds after `pb-ap` was confirmed back via the kernel log), likely a kernfs/sysfs inotify limitation, not proven at the kernel-internals level. (3) **Confirmed working differently**: `RUN+="/usr/bin/systemctl --no-block start hostapd.service"` added directly to the same udev `add`-event rule line that has reliably renamed the interface across every blip observed (four independent re-enumerations, zero rename failures) - runs inside udev's own already-proven event processing, with no dependency on a later event or a virtual filesystem's inotify behavior. `override.conf`'s stop-half (`BindsTo=`/`ConditionPathExists=`) unchanged, reconfirmed working on three real ALFA blips today. 16/16 regression tests pass (`tools/test_hostapd_recovery_config.py`, the dead `.path` unit's tests replaced with `RUN+=`-specific ones). | Mechanism #3 is committed to `main` but the live Pi still has mechanism #2 (the non-working `.path` unit) installed as of the last check - needs: redeploy the corrected udev rule, remove the dead `.path` unit live, reload udev rules, then one more real or natural disconnect/reconnect cycle to confirm `RUN+=` actually fires hostapd where the first two did not. Exact commands in `docs/OPERATIONAL-DECISIONS.md`'s dated entry | `docs/OPERATIONAL-DECISIONS.md` 2026-09-15 dated entry and its same-day update, 2026-09-05 incident entry for prior history |
