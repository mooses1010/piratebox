...
| hostapd auto-recovery: udev `SYSTEMD_WANTS` restart trigger unreliable | **DOCUMENTED ONLY, not yet fixed** - real gap found 2026-09-05, same day as the fix it's part of | The 2026-09-05 hostapd runtime-reattachment fix's `BindsTo=`/`ConditionPathExists=` half (clean stop when `pb-ap` disappears) is confirmed working on a real re-enumeration; its other half (`ENV{SYSTEMD_WANTS}+="hostapd.service"` in the udev rule, meant to restart hostapd when the interface reappears) did **not** fire on a real ALFA blip during unrelated RTL-SDR work, leaving the AP down until a manual `sudo systemctl restart hostapd`. The rule itself was confirmed byte-correct via `udevadm test` immediately after; root cause of why the real event didn't trigger a start job is unestablished (needs a root-level udev debug trace, not pursued to avoid turning an unrelated task into a second hostapd project) | A manual `sudo systemctl restart hostapd` remains the reliable fallback whenever the SSID doesn't reappear on its own after an ALFA blip. If picked up as its own round, most likely replacement: a `.path` unit (`PathExists=/sys/class/net/pb-ap`) - a more battle-tested, inotify-backed mechanism than udev's per-rule `SYSTEMD_WANTS` property translation | `docs/OPERATIONAL-DECISIONS.md` 2026-09-05 incident entry, "New known issue found after closure" |
