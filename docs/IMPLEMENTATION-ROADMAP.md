...
| ESP32-S3 hardware/sensor supervisor | **PARTIALLY IMPLEMENTED (2026-09-07, DS18B20 phase)** - the deployed foundation (identity, firmware+daemon stack, BH1750 migration) is IMPLEMENTED+DEPLOYED+LIVE-VERIFIED (checkpoint `86f256a`); multi-probe DS18B20 support is fully built/tested but genuinely BLOCKED BY HARDWARE (not wired) | Everything from the prior checkpoint remains live (`temp_internal` + `bh1750` capabilities, Environment UI, `HARDWARE_SIGNALS`). New this phase, all built and tested but not yet deployed/wired: DS18B20 multi-probe firmware (`esp32-firmware/include/ds18b20.h`/`.cpp`, GPIO4, non-blocking, real ROM-address bus search), a naming/commissioning workflow (`piratebox_ds18b20_roles.py`, `tools/ds18b20_commission.py`, no new sudoers grant - uses `moose`'s existing `gpio` group), Environment UI for named probes, an OLED "probe" glance page, and ambient-light-driven OLED auto-brightness (deployed independently of DS18B20 - reuses the already-migrated BH1750 signal, no new hardware). See `docs/ESP32-SUPERVISOR-DESIGN.md` §17-20 for the full design, including the onboard-RGB and black-box-event-log evaluations (deferred/not-implemented with reasons, not silently skipped). 25+6+6+19 new Python tests, 13 new PHP assertions, all passing; full project-wide regression clean | **Physical wiring gate**: four owned DS18B20 probes need connecting (VCC→3.3V, GND→GND, DATA→GPIO4) - not performed, per standing instruction not to request wiring before software is ready. Once wired: flash the DS18B20-aware firmware, commission/name probes, redeploy the updated OLED daemon (auto-brightness + probe glance page - one operator `sudo cp`+restart, not yet done). BME280/INA226/power-supervision remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md`, `docs/CHECKPOINTS.md` |
