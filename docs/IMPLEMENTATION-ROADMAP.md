...
|---|---|---|---|---|
| Admin/status page + auth gate | IMPLEMENTED IN REPO, NOT YET DEPLOYED (mechanism itself already IMPLEMENTED+DEPLOYED+LIVE-VERIFIED) | **2026-09-02: real operator usability gap found and fixed** - operator hit the nginx Basic Auth prompt at `/admin/` with no known credentials. Full audit (`docs/OPERATIONAL-DECISIONS.md` "Admin Panel Auth Readiness") found the underlying mechanism (`setup_admin_password.sh`, openssl-based, no missing dependency, deploy-excluded, reinstall-safe, correct permissions) was already fully correct - the gap was pure rediscoverability. Fixed: new `admin_panel` capability (self-description, `DEGRADED` not `UNAVAILABLE` when unconfigured - the secure default, not a fault), a `help.php` mention reachable pre-auth for a first-time operator, and an honest About-page wording tweak - without exposing credentials/paths to public clients (About only ever shows aggregate counts, never per-capability labels) | **Two operator actions required** (see docs/OPERATIONAL-DECISIONS.md for exact commands): (1) `sudo /usr/local/bin/setup_admin_password.sh` to actually set the first password - a genuinely interactive step Claude Code must never perform; (2) reinstall the updated `piratebox_status_helper.sh` to `/usr/local/bin/` for the new self-description field to take effect | Phase 4 entry, `docs/OPERATIONAL-DECISIONS.md` "Admin Panel Auth Readiness" |
