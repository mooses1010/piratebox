...
| ESP32-S3 hardware/sensor supervisor | **PARTIALLY IMPLEMENTED (2026-09-07)** - identity commissioned, full firmware+daemon stack built and live-validated, first real sensor (BH1750) migrated and validated | PlatformIO/Arduino firmware (NDJSON protocol, task watchdog, `temp_internal` + `bh1750` capabilities), Pi daemon (`piratebox_esp32_supervisor.py`, stable `/dev/serial/by-id/` discovery), `HARDWARE_SIGNALS` registration (`esp32_temp_internal`, and `ambient_lux` now ESP32-owned via `piratebox_esp32_bh1750.py`), Environment UI section, factory firmware backed up and independently verified before first custom flash - all built and validated on the real commissioned board. BH1750 physically migrated from the Pi's I2C bus to the ESP32's own (GPIO8/9) - a real mid-migration wiring fault was found and fixed (see `docs/OPERATIONAL-DECISIONS.md`). 400 Python + 418 PHP tests passing project-wide, zero regressions. Requires an externally-powered USB hub in the current prototype topology (unrelated to, but correlated with, the Pi's own still-unresolved `0x50005` undervoltage condition) | Redeploy the updated `piratebox_oled_daemon.py` to the live `piratebox-oled.service` and restart `piratebox-esp32-supervisor.service` (both need one sudo step each, consolidated in the operator report). DS18B20/BME280/INA226/power-supervision remain architecture-only, no hardware wired | `docs/ESP32-SUPERVISOR-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md` |
