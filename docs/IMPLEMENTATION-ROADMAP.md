...
| ESP32-S3 hardware/sensor supervisor | **IMPLEMENTED + DEPLOYED + LIVE-VERIFIED (2026-09-07)** - identity commissioned, full firmware+daemon stack built, BH1750 physically migrated and confirmed live end-to-end | PlatformIO/Arduino firmware (NDJSON protocol, task watchdog, `temp_internal` + `bh1750` capabilities), Pi daemon (`piratebox_esp32_supervisor.py`, stable `/dev/serial/by-id/` discovery), `HARDWARE_SIGNALS` registration (`esp32_temp_internal`, and `ambient_lux` now ESP32-owned via `piratebox_esp32_bh1750.py`), Environment UI (both the Ambient Light and Hardware Supervisor sections confirmed rendering real live data), factory firmware backed up and independently verified before first custom flash. BH1750 physically migrated from the Pi's I2C bus to the ESP32's own (GPIO8/9) - two real faults (a wiring fault, and a missing PHP `open_basedir` entry) were found, diagnosed from evidence, and fixed - see `docs/OPERATIONAL-DECISIONS.md` and `docs/CHECKPOINTS.md`'s entry for `86f256a`. 400 Python + 418 PHP tests passing project-wide, zero regressions. Independently confirmed by the operator: services active, real sensor data, Pi I2C bus correctly clear of the migrated sensor. Requires an externally-powered USB hub in the current prototype topology (unrelated to, but correlated with, the Pi's own still-unresolved `0x50005` undervoltage condition) | None pending - this round's work is complete. DS18B20/BME280/INA226/power-supervision remain architecture-only pending new hardware the operator hasn't acquired yet; not to be started speculatively | `docs/ESP32-SUPERVISOR-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md`, `docs/CHECKPOINTS.md` |
