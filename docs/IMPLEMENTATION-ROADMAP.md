...
| ALFA AWUS036ACM (MT7612U) upgrade | HARDWARE + 2.4/5GHz VALIDATED, REGULATORY DOMAIN FIXED, MIGRATION ARCHITECTURE STAGED - not yet production AP | `lsusb` confirms `0e8d:7612`/`mt76x2u` bound; `iw reg get` independently confirmed `country US: DFS-FCC` post-reboot (the documented one-command fix needed a reboot too - live-apply proved broken on this system); 5GHz AP validated on non-DFS channel 36 (`AP-ENABLED`, WPA2 handshake completed 2x); stable `pb-ap` naming, radio role model, boot fallback, NetworkManager ownership fix, migration+rollback scripts all staged but NOT installed/executed; `wlan0` confirmed untouched throughout | operator gate: meet the power-aware soak gate (still not met - only short single-client tests exist so far), then explicitly run the staged migration script | `docs/EXTERNAL-AP-ARCHITECTURE-DESIGN.md`, `docs/OPERATIONAL-DECISIONS.md` "AWUS036ACM Hardware Validation Round" / "External AP Architecture + Production Migration Readiness Round" / "Regulatory Domain Correction + ALFA Post-Regulatory Validation Round" |
