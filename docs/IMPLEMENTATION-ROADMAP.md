...
| OLED (SSD1306, I2C) | IMPLEMENTED+DEPLOYED+LIVE-VERIFIED (2026-09-03); Silly Mode + Progression added 2026-09-04; Captain's Log web profile added 2026-09-04 | I2C1 enabled (`dtparam=i2c_arm=on` + `i2c-dev` module, both persistent); `i2cdetect -y 1` confirms the display at 0x3C; a real frame written and visually confirmed by the operator; `piratebox-oled.service` running, four-page rotation (Status/Time/Network/Health) confirmed live on the physical display. Silly Mode: optional, user-toggled (`piratebox-silly {on,off,status}`), default off, expressive face/personality display state subordinate to Emergency Mode/faults. Progression: persistent XP/level/title/achievement/lifetime-history layer underneath Silly Mode (`piratebox_progression.py`, durable at `/var/lib/piratebox-oled/`), cosmetic-only rewards, anti-farming and privacy by construction, with a currently-empty `HARDWARE_SIGNALS` extension registry for the DS3231/INA226/BME280/DS18B20/BH1750/RGB/GPS row below. Captain's Log: read-only web page (`/utility/captains-log/`) presenting Progression to visitors, via a new root-published public export (no filesystem permission loosened; one narrow named-file `open_basedir` addition instead) - see `docs/OPERATIONAL-DECISIONS.md` → "OLED Silly Mode" / "PirateBox Progression" / "Captain's Log web profile" | none for the OLED itself - GPIO22/23 (cycle/wake buttons) remain BLOCKED BY HARDWARE, see the row below | HARDWARE-INTEGRATION-DESIGN.md §12, PHYSICAL-CONTROL-UX-DESIGN.md §1, OPERATIONAL-DECISIONS.md "OLED Silly Mode" / "PirateBox Progression" / "Captain's Log web profile" |
