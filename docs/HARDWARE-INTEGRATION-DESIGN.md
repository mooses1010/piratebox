...
| DS3231 RTC + AT24C32 EEPROM | GPIO2/GPIO3 (shared) | 3/5 (shared) | **WIRED, VERIFIED, BATTERY-BACKED (2026-09-07)** | Same I2C1 bus/pins as the OLED above - multi-drop, no pin conflict, exactly as anticipated when this row was first written. Confirmed via `i2cdetect`: RTC at 0x68 (`UU` once driver-bound), its onboard EEPROM at 0x57 (not the same as, and no conflict with, the separate HAT-ID EEPROM bus on GPIO0/1). OLED (0x3c) confirmed unaffected. **Production module has R4 (200 ohm charging resistor) removed and a real CR2032 installed** - measured ~3.14-3.18V→~0.22V across the battery holder before/after R4's removal, confirming the charging path (unsafe for a non-rechargeable cell) is genuinely broken. Kernel driver bring-up/config: `docs/RTC-TIME-READINESS-DESIGN.md` §6-§8 (§8 has the R4/CR2032 record and the pending physical power-loss validation test). |
