...
| DS3231 RTC + AT24C32 EEPROM | GPIO2/GPIO3 (shared) | 3/5 (shared) | **WIRED, VERIFIED WHILE POWERED - BATTERY BACKUP UNCONFIRMED, FAILED FIRST TEST (2026-09-07)** | Same I2C1 bus/pins as the OLED above - multi-drop, no pin conflict, exactly as anticipated when this row was first written. Confirmed via `i2cdetect`: RTC at 0x68 (`UU` once driver-bound), its onboard EEPROM at 0x57 (not the same as, and no conflict with, the separate HAT-ID EEPROM bus on GPIO0/1). OLED (0x3c) confirmed unaffected throughout. **Production module has R4 (200 ohm charging resistor) removed and a CR2032 installed**, but the first genuine total-power-loss test **failed**: the chip came back reading its factory power-on default instead of advancing time from the battery - see `docs/RTC-TIME-READINESS-DESIGN.md` §9 for the evidence and the physical measurement plan now in progress. Do not treat this row as "battery-backed" until a real power-loss test passes. |
