...
| DS3231 RTC + AT24C32 EEPROM | GPIO2/GPIO3 (shared) | 3/5 (shared) | **WIRED, VERIFIED, 2026-09-06** | Same I2C1 bus/pins as the OLED above - multi-drop, no pin conflict, exactly as anticipated when this row was first written. Confirmed via `i2cdetect`: RTC at 0x68, its onboard EEPROM at 0x57 (not the same as, and no conflict with, the separate HAT-ID EEPROM bus on GPIO0/1). OLED (0x3c) confirmed unaffected. Kernel driver bring-up/config: `docs/RTC-TIME-READINESS-DESIGN.md` §6. No coin cell installed yet (see that section). |
