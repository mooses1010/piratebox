...
| BH1750 ambient light | ~~GPIO2/GPIO3 (shared)~~ **MIGRATED OFF THE PI (2026-09-07)** | ~~3/5 (shared)~~ | **NO LONGER WIRED TO THE PI** | Originally commissioned on the Pi's own I2C1 bus (2026-09-07) - confirmed via `i2cdetect` at address 0x23 alongside the OLED (0x3c)/RTC (0x68)/EEPROM (0x57), all then-healthy. **Physically migrated the same day** to the ESP32-S3 supervisor's own I2C bus (GPIO8/SDA, GPIO9/SCL - see `docs/ESP32-SUPERVISOR-DESIGN.md` §7a/§16) once the supervisor's firmware/Pi-daemon/capability system were built and validated. `i2cdetect -y 1` on the Pi now correctly shows **no device at 0x23** - this is the expected post-migration state, not a fault. `piratebox_bh1750.py` (the original Pi-side reader) is unchanged and left in the repo for historical/rollback reference, but is no longer imported by anything; `piratebox_esp32_bh1750.py` is its drop-in replacement, reading the sensor's value from the ESP32 supervisor's cached export instead of directly from I2C. See `docs/OPERATIONAL-DECISIONS.md` for the full commissioning AND migration record, including a real mid-migration wiring fault (found, fixed, and documented) and the post-migration validated reading. |
...
`docs/ESP32-SUPERVISOR-DESIGN.md` §7a. **Update (2026-09-07, same day):
the BH1750 ambient light sensor (see its own row above) was physically
