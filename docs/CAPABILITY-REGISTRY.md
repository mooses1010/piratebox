...
| Remote microcontroller/sensor node (e.g. ESP32) | Optional/Field | **ESP32-S3-N16R8 has enumerated successfully (`303a:4001` and, on later resets, the ROM's own `303a:1001`), but is now cycling disconnect/reconnect unpredictably (2026-09-07)** - `connect-debounce failed` has now also appeared on its current port (previously seen only on the original, different port), and the board is intermittently absent from the bus entirely. Correlated with a live undervoltage condition (`0x50005`), not yet proven caused by it. Flash/PSRAM/ROM identity still unverified - `esptool` identified as available (Debian package, not yet installed) but a stable connection window is needed first. | Network Companion (pending re-evaluation - board connects via USB, not LAN) |
